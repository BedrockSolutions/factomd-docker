...
const writeConfigFile = async (config, tmpDir) =>
  writeFile(join(tmpDir, 'values.yaml'), safeDump(config))

const copyFileFromContainer = async (filename, tmpDir) => {
  const name = crypto.randomBytes(5).toString('hex')
  try {
    await execAsync(
      `docker run --name ${name} -v ${tmpDir}:/app/config bedrocksolutions/factomd:latest true`
    )
    await execAsync(
      `docker cp ${name}:${join('/app', filename)} ${join(tmpDir, filename)}`
    )
...
const runConfz = async config => {
  ...
  try {
    ...
    const name = crypto.randomBytes(5).toString('hex')
    await execAsync(
      `docker run --name ${name} -v ${tmpDir}:/app/config bedrocksolutions/factomd:latest ./bin/confz run`
    )
  } finally {
    if (tmpDir) {
      await removeTmpDir(tmpDir)
